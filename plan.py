...
class Planner(Singleton):
    ...
    def extract(self, text):
        ...
        for i in range(len(text)):
            for j in range(2):
                tmp.add(text[i:i+j + 1])
        for sentence in split_sentences(text):
            # 过滤器：过滤出既在text中，又在ranked_words中的词语
            keywords.update(
                filter(lambda w: w in self.ranked_words and self.ranked_words.rank[w] < 10000, tmp))
        abandon = set()
        for t in keywords:
            if len(t) >= 2 and (t[0] in keywords or t[1] in keywords):
                abandon.add(t[0])
        ret = set()
        for t in keywords:
            if t in abandon:
    ...
    def plan(self, text):
        ...
        for i in filtered_keywords:
            result.append(i)
        while len(result) < 4:  # 存在在model中的keywords
